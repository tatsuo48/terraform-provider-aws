...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        AwsProvider(self, "aws",
            assume_role_with_web_identity=[AwsProviderAssumeRoleWithWebIdentity(
                role_arn="arn:aws:iam::123456789012:role/ROLE_NAME",
                session_name="SESSION_NAME",
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        ...
        TerraformOutput(self, "vpc_all_tags",
            value=example.tags_all
        )
        TerraformOutput(self, "vpc_resource_level_tags",
            value=example.tags
        )
```
...
<!-- cache-key: cdktf-0.18.0 input-cad02f5e7a2db4b39ff182d925e9634bb2dd029cd0704a458af6bd01c2faae8f -->